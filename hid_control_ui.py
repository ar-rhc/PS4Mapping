...
class MappingConfigFrame(tk.Frame):
    ...
    def create_visual_ui(self):
        """Builds the main UI with canvas, controller image, and buttons."""
        ...
        try:
            ...
            img_x = (950 - new_width) // 2
            img_y = (700 - new_height) // 2
            self.canvas.create_image(img_x, img_y, anchor='nw', image=self.ps4_img)
            self.scaled_buttons = {btn: (img_x + int(x * scale_factor), img_y + int(y * scale_factor)) for btn, (x, y) in self.BUTTONS.items()}
        except Exception as e:
            self.ps4_img = None
            self.canvas.create_text(475, 350, text=f"[PS4 image not found]\n{e}", font=("Arial", 16), fill="red")
            self.scaled_buttons = self.BUTTONS.copy()
        for btn, (x, y) in self.scaled_buttons.items():
            w = Button(self, text=self.BUTTON_LABELS[btn], width=60, height=30, font=("Arial", 11, "bold"), fg="white", bg="#808080", activebackground="#808080", activeforeground="white", highlightthickness=0, borderwidth=0)
            ...
            key_label = tk.Label(self, text="", font=("Arial", 10, "bold"), fg="white", bg="#808080", height=1, relief="flat", anchor="center")
            self.key_labels[btn] = key_label
            self.canvas.create_window(x, y + 22, window=key_label, width=60)
        # MODIFIED: Button now just saves, as Hammerspoon reloads automatically.
        self.save_btn = Button(self, text="Save Mappings", command=self.save_mappings, bg="#808080", fg="white", font=("Arial", 12, "bold"))
        self.canvas.create_window(850, 650, window=self.save_btn)
        # MODIFIED: Info label text is updated.
        self.info_label = tk.Label(self, text="Click to map Keystroke. Double-click to edit or map BTT Trigger.", font=("Arial", 10, "bold"), fg="black", bg="white")
        self.canvas.create_window(475, 680, window=self.info_label)

    def edit_mapping(self, btn):
        """REPLACED: Opens an advanced editing window for a button, allowing selection between Keystroke and BTT Trigger."""
        ...
        top.title(f"Edit Mapping: {self.BUTTON_LABELS[btn]}")
        top.configure(bg="#222")
        top.transient(self)
        top.grab_set()

        keystroke_frame = tk.LabelFrame(top, text="Map to Keystroke", bg="#222", fg="white", padx=10, pady=10)
        keystroke_frame.pack(pady=10, padx=10, fill="x")
        
        mods = {m: tk.BooleanVar(value=current_mapping.get('modifiers', {}).get(m, False)) for m in ['ctrl', 'opt', 'shift', 'cmd']}
        
        mod_frame = tk.Frame(keystroke_frame, bg="#222")
        mod_frame.pack(pady=5)
        for m in ['ctrl', 'opt', 'shift', 'cmd']:
            tk.Checkbutton(mod_frame, text=m.capitalize(), variable=mods[m], bg="#222", fg="white", selectcolor="#444").pack(side='left')
        
        # Display current key as non-editable label
        key_display = tk.Label(keystroke_frame, text=current_mapping.get('key', 'No key mapped'), font=("Arial", 12, "bold"), fg="white", bg="black", width=10, relief="sunken")
        key_display.pack(pady=5)
        
        # Add a button to map key
        def map_key_in_edit():
            top.withdraw()  # Hide the edit window temporarily
            self.info_label.config(text=f"Press a key for '{self.BUTTON_LABELS[btn]}'... (Esc to cancel)", fg="#1976D2")
            
            # Store the button we're capturing for
            ...
            self._key_capture_profile = active_profile
            ...
            # Override the finish function to update the edit window
            original_finish = self._finish_key_capture
            def custom_finish():
                original_finish()
                # Update the key display in the edit window
                new_mapping = self.mappings[active_profile][section][mapping_key]
                if new_mapping and 'key' in new_mapping:
                    key_display.config(text=new_mapping['key'])
                    # Update modifiers
                    for m, var in mods.items():
                        var.set(new_mapping.get('modifiers', {}).get(m, False))
                    # Clear BTT trigger text since we now have a key mapping
                    btt_var.set("")
                top.deiconify()  # Show the edit window again
                self.info_label.config(text="Click to map Keystroke. Double-click to edit or map BTT Trigger.", fg="black")
            
            self._finish_key_capture = custom_finish
        
        map_key_btn = Button(keystroke_frame, text="Map Key", command=map_key_in_edit, bg="#1976D2", fg="white", font=("Arial", 10, "bold"))
        map_key_btn.pack(pady=5)

        btt_frame = tk.LabelFrame(top, text="OR Map to BTT Named Trigger", bg="#222", fg="white", padx=10, pady=10)
        btt_frame.pack(pady=10, padx=10, fill="x")
        
        btt_var = tk.StringVar(value=current_mapping.get('bttnamekey', ''))
        btt_entry = tk.Entry(btt_frame, textvariable=btt_var, font=("Arial", 12, "bold"), width=20, fg="white", bg="black")
        btt_entry.pack(pady=5)
        btt_entry.focus_set()
        
        # Add toggle for applying to all profiles
        apply_all_frame = tk.Frame(top, bg="#222")
        apply_all_frame.pack(pady=10, padx=10, fill="x")
        
        apply_all_var = tk.BooleanVar(value=False)
        apply_all_check = tk.Checkbutton(apply_all_frame, text="Apply this mapping to ALL profiles", 
                                       variable=apply_all_var, bg="#222", fg="white", selectcolor="#444",
                                       font=("Arial", 10, "bold"))
        apply_all_check.pack(side='left')
        
        # Add info about the toggle
        tk.Label(apply_all_frame, text="(Useful for common mappings like navigation keys)", 
                font=("Arial", 8), fg="gray", bg="#222").pack(side='right')
        ...
        def save():
            ...
            elif current_mapping.get('key'):  # Keep existing key mapping if no BTT name
                new_mapping = {'modifiers': {k: v.get() for k, v in mods.items()}, 'key': current_mapping.get('key')}
            
            if new_mapping:
                if apply_all_var.get():
                    # Apply to all profiles
                    for profile_name in self.mappings.keys():
                        if section not in self.mappings[profile_name]:
                            self.mappings[profile_name][section] = {}
                        self.mappings[profile_name][section][mapping_key] = new_mapping.copy()
                    print(f"✅ Applied mapping to ALL profiles: {mapping_key}")
                else:
                    # Apply only to current profile
                    self.mappings[active_profile][section][mapping_key] = new_mapping
            else:
                if apply_all_var.get():
                    # Remove from all profiles
                    for profile_name in self.mappings.keys():
                        if section in self.mappings[profile_name] and mapping_key in self.mappings[profile_name][section]:
                            del self.mappings[profile_name][section][mapping_key]
                    print(f"✅ Removed mapping from ALL profiles: {mapping_key}")
                else:
                    # Remove only from current profile
                    unmap(ask_confirm=False)

            self.update_button_colors()
            self.save_mappings()
            top.destroy()

        def unmap(ask_confirm=True):
            if apply_all_var.get():
                # Remove from all profiles
                for profile_name in self.mappings.keys():
                    if section in self.mappings[profile_name] and mapping_key in self.mappings[profile_name][section]:
        ...
        button_frame = tk.Frame(top, bg="#222")
        button_frame.pack(pady=10)
        Button(button_frame, text="Save", command=save, bg="#4CAF50", fg="white").pack(side='left', padx=5)
        Button(button_frame, text="Unmap", command=unmap, bg="#D32F2F", fg="white").pack(side='left', padx=5)
        Button(button_frame, text="Cancel", command=top.destroy, bg="#808080", fg="white").pack(side='left', padx=5)

    def update_button_colors(self):
        """REPLACED: Updates button colors and labels for Keystrokes (Green) and BTT Triggers (Blue)."""
        active_profile = self.active_profile_name
        profile_map = self.mappings.get(active_profile, {"buttons": {}, "dpad": {}})
        for btn, w in self.button_widgets.items():
            section = "dpad" if btn.startswith('dpad_') else "buttons"
            mapping_key = btn.replace('dpad_', '') if section == "dpad" else btn
            mapping = profile_map.get(section, {}).get(mapping_key)
            key_label = self.key_labels[btn]
            if mapping:
                if 'bttnamekey' in mapping and mapping['bttnamekey']:
                    w.config(bg="#2196F3", activebackground="#2196F3") # Blue for BTT
                    key_text = f"BTT: {mapping['bttnamekey']}"
                    if len(key_text) > 10: key_text = key_text[:8] + '..'
                    key_label.config(text=key_text, fg="white", bg="#2196F3")
                elif 'key' in mapping and mapping['key']:
                    w.config(bg="#4CAF50", activebackground="#4CAF50") # Green for Keystroke
                    key_text = mapping['key']
                    mods = mapping.get('modifiers', {})
                    mod_indicators = [m.capitalize()[0] for m in ['cmd', 'ctrl', 'opt', 'shift'] if mods.get(m)]
                    if mod_indicators:
                        key_text = '+'.join(mod_indicators) + '+' + key_text
                    if len(key_text) > 8: key_text = key_text[:6] + '..'
                    key_label.config(text=key_text, fg="white", bg="#4CAF50")
                else:
                    w.config(bg="#808080", activebackground="#808080")
                    key_label.config(text="", bg="#808080")
            else:
                w.config(bg="#808080", activebackground="#808080")
                key_label.config(text="", bg="#808080")
    ...
    def load_mappings(self):
        """MODIFIED: Loads mappings from the JSON file."""
    ...
    def save_mappings(self):
        """REPLACED: This is the new primary save function. It writes to the JSON file."""
        try:
            with open(self.MAPPING_FILE, 'w') as f:
                json.dump(self.mappings, f, indent=4)
            self.info_label.config(text="Mappings saved! Hammerspoon will auto-reload.", fg="#4CAF50")
            print("✅ Mappings saved to mappings.json")
        except IOError as e:
            self.info_label.config(text=f"Error saving mappings: {e}", fg="red")
            print(f"Error saving mappings: {e}")
        self.after(3000, lambda: self.info_label.config(text="Click to map Keystroke. Double-click to edit or map BTT Trigger.", fg="black"))

    def start_key_capture(self, btn):
        """MODIFIED: This now only handles quick keystroke mapping. Double-click for BTT."""
        self.info_label.config(text=f"Press a key for '{self.BUTTON_LABELS[btn]}'... (Esc to cancel)", fg="#1976D2")
        ...
        # Bind to the main window instead of creating a new one
        self.winfo_toplevel().bind_all('<KeyPress>', self._on_key_press)
        self.winfo_toplevel().bind_all('<KeyRelease>', self._on_key_release)
    ...
    def _on_key_press(self, event):
        ...
        if not hasattr(self, '_key_capture_active') or not self._key_capture_active:
            return
    ...
    def _cleanup_key_capture(self):
        ...
        if hasattr(self, '_key_capture_active') and self._key_capture_active:
            ...
            self.after(2000, lambda: self.info_label.config(text="Click to map Keystroke. Double-click to edit or map BTT Trigger.", fg="black"))
    ...
    def add_profile(self):
        ...
        if platform.system() == 'Darwin':
            try:
                # Get running applications with better filtering
                script = '''
                tell application "System Events"
                    set appList to {}
                    repeat with proc in (every process whose background only is false)
                        set appName to name of proc
                        if appName is not in {"Finder", "Dock", "SystemUIServer", "WindowServer", "loginwindow", "UserEventAgent", "launchd", "kernel_task"} then
        ...
        info_label = tk.Label(dialog, text="💡 Tip: Use the app name as shown in the menu bar.\nFuzzy matching will handle variations.", 
        ...
        listbox = tk.Listbox(list_frame, font=("Arial", 11), height=15, width=40, yscrollcommand=scrollbar.set)
        ...
        ok_btn = Button(button_frame, text="Add Profile", command=on_ok, bg="#4CAF50", fg="white")
        ok_btn.pack(side='left', padx=5)
        
        cancel_btn = Button(button_frame, text="Cancel", command=dialog.destroy, bg="#808080", fg="white")
...
class DS4ControlUI(tk.Tk):
    def __init__(self):
        ...
        # OPTIMIZATION: Create a dedicated queue for UDP packets
        self.udp_queue = queue.Queue()
        self.udp_thread = threading.Thread(target=self._udp_sender_thread, daemon=True)
        self.udp_thread.start()
        
        # --- Event-Driven UDP: Track previous digital state ---
        self.previous_digital_state = {
            'buttons': {},
            'dpad': 'none'
        }
        self.udp_packets_sent = 0
        self.udp_packets_saved = 0
        # -----------------------------------------------------
        
# --- ADD THE CODE BELOW ---
    # This is the NEW socket for LISTENING for the toggle command
        self.ipc_port = 12346
        ...
        try:
            ...
            messagebox.showerror("Port Error", f"Could not bind to port {self.ipc_port}. Is another instance running?\n\nError: {e}")
            self.destroy()
            return
# --- ADD THE CODE ABOVE ---
...
# --- ADD THE CODE BELOW ---
    def check_ipc_socket(self):
        """Checks for incoming UDP commands from Hammerspoon."""
        try:
    ...
    def poll_controller(self):
        ...
        try:
            ...
            if report:
                ...
                # Send data to Hammerspoon via UDP (JSON for stability - testing)
                try:
                    digital_changes = self.check_digital_changes(controller_data)
                    if digital_changes:
                        # TEMPORARY: Use JSON for stable UDP communication
                        essential_data = {
                            'buttons': digital_changes['buttons'],
                            'dpad': digital_changes['dpad'],
                            'timestamp': time.time()
                        }
                        message = json.dumps(essential_data).encode('utf-8')
                        self.udp_socket.sendto(message, (self.udp_host, self.udp_port))
                        
                        # OPTIMIZATION: For critical buttons, send immediately without waiting
                        critical_buttons = ['cross', 'circle', 'triangle', 'square', 'options']
                        has_critical = any(btn in digital_changes['buttons'] for btn in critical_buttons)
                        
                        if has_critical:
                            # Send a second packet immediately for critical buttons
                            self.udp_socket.sendto(message, (self.udp_host, self.udp_port))
                        
                        self.udp_packets_sent += 1
                    else:
                        self.udp_packets_saved += 1
    ...
    def check_digital_changes(self, new_data):
        ...
        for button, is_pressed in new_data['buttons'].items():
            ...
            if is_pressed != was_pressed:
                changes['buttons'][button] = is_pressed
                has_changes = True
        
        # Check d-pad changes
        if new_data['dpad'] != self.previous_digital_state['dpad']:
            changes['dpad'] = new_data['dpad']
            has_changes = True
...
UDP OPTIMIZATION (JSON - Stable Testing):
Packets Sent: {self.udp_packets_sent}
Packets Saved: {self.udp_packets_saved}
Efficiency: {(self.udp_packets_saved / max(1, self.udp_packets_sent + self.udp_packets_saved) * 100):.1f}%
Packet Size: ~300 bytes JSON (Binary: 8 bytes)
Polling Rate: {self.polling_rate}ms (~{1000//self.polling_rate} Hz)
"""
    ...
    def load_settings(self):
        ...
        default_settings = {
            ...
            'damping_factor': 0.95,
            'polling_rate': 8,   # Ultra-low latency: 8ms (~120Hz) for immediate response
...
# --- ADD THE CODE BELOW ---
        if hasattr(self, 'ipc_socket'):
            self.ipc_socket.close() # The new listening socket
# --- ADD THE CODE ABOVE ---
#      
        self.destroy()
